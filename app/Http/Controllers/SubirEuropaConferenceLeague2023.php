...
use App\Models\World\EuropaConferenceLeague2023;
use App\Models\World\EuropaConferenceLeagueStat2023;
use App\Models\Log;
...
class SubirEuropaConferenceLeague2023 extends Controller
{
        ...
        if ($response->ok()) {
            ...
            if (isset($data['response']) && is_array($data['response'])) {
                ...
                foreach ($data['response'] as $fixtureData) {
                    ...
                    if (!EuropaConferenceLeague2023::where('fixture_id', $fixtureData['fixture']['id'])->exists() || !EuropaConferenceLeagueStat2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                        ...
                        if (!EuropaConferenceLeague2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                            $contadorPartidos++;
                        
                            $fixtures = EuropaConferenceLeague2023::create([
                        ...
                        if (!EuropaConferenceLeagueStat2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                            ...
                            if ($response2->ok()) {
                                ...
                                if (isset($data2['response']) && is_array($data2['response'])) {
                                    ...
                                    foreach ($data2['response'] as $indice => $fixtureData2) {
                                        if (!EuropaConferenceLeagueStat2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                                            $contadorStats++;
                                            EuropaConferenceLeagueStat2023::create([
                                                'fixture_date' => $fechaFormateada,
                                                'fixture_id' => $fixtureData['fixture']['id'],
                                                'fixture_referee' =>  $fixtureData['fixture']['referee'],
                                                'fixture_timestamp' => $fechaHora,
