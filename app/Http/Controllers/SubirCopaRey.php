...
class SubirCopaRey extends Controller
{
        ...
        if ($response->ok()) {
            ...
            if (isset($data['response']) && is_array($data['response'])) {
                ...
                foreach ($data['response'] as $fixtureData) {
                    ...
                    if (!CopaRey2023::where('fixture_id', $fixtureData['fixture']['id'])->exists() || !CopaReyStat2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                        ...
                        if (!CopaRey2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                            $contadorPartidos++;
                            //se rellena la tabla premierleaguefixture
                            $fixtures = CopaRey2023::create([
                        ...
                        /*
                        //Estas rondas no estan cubiertas en la api, asi que es mejor excluirlas
                        $excludedRounds = ['Preliminary Round', '1st Round', '2nd Round','Round of 32'];

                        if (!in_array($fixtureData['league']['round'], $excludedRounds)) {
                            if (!CopaReyStat2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
