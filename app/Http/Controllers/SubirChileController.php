...
use App\Models\Chile\Chile_primera_division2024;
use App\Models\Chile\PrimeraDivisionStat2024;
use App\Models\Log;
use Illuminate\Support\Facades\Http;
use Carbon\Carbon;
use Twilio\Rest\Client;

class SubirChileController extends Controller
{
        ...
        if ($response->ok()) {
            ...
            if (isset($data['response']) && is_array($data['response'])) {
                ...
                foreach ($data['response'] as $fixtureData) {
                    ...
                    if (!Chile_primera_division2024::where('fixture_id', $fixtureData['fixture']['id'])->exists() || !PrimeraDivisionStat2024::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                        ...
                        if (!Chile_primera_division2024::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                            $contadorPartidos++;
                            //se rellena la tabla premierleaguefixture
                            $fixtures = Chile_primera_division2024::create([
                        ...
                        if (!PrimeraDivisionStat2024::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                            ...
                            if ($response2->ok()) {
                                ...
                                if (isset($data2['response']) && is_array($data2['response'])) {
                                    ...
                                    foreach ($data2['response'] as $indice => $fixtureData2) {
                                        if (!PrimeraDivisionStat2024::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                                            $contadorStats++;
                                            PrimeraDivisionStat2024::create([
                                                'fixture_date' => $fechaFormateada,
                                                'fixture_id' => $fixtureData['fixture']['id'],
                                                'fixture_referee' =>  $fixtureData['fixture']['referee'],
                                                'fixture_timestamp' => $fechaHora,
