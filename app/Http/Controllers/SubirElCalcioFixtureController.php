...
use App\Models\Italia\SubirElCalcioFixture2023;
use App\Models\Italia\FixtureElCalcioStat2023;
use App\Models\Log;
use Illuminate\Support\Facades\Http;
use Carbon\Carbon;
use Twilio\Rest\Client;

class SubirElCalcioFixtureController extends Controller
{
        ...
        if ($response->ok()) {
            ...
            if (isset($data['response']) && is_array($data['response'])) {
                ...
                foreach ($data['response'] as $fixtureData) {
                    ...
                    if (!SubirElCalcioFixture2023::where('fixture_id', $fixtureData['fixture']['id'])->exists() || !FixtureElCalcioStat2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                        ...
                        if (!SubirElCalcioFixture2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                            $contadorPartidos++;
                            //se rellena la tabla premierleaguefixture
                            $fixtures = SubirElCalcioFixture2023::create([
                        ...
                        if (!FixtureElCalcioStat2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                            ...
                            if ($response2->ok()) {
                                ...
                                if (isset($data2['response']) && is_array($data2['response'])) {
                                    ...
                                    foreach ($data2['response'] as $indice => $fixtureData2) {
                                        if (!FixtureElCalcioStat2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                                            $contadorStats++;
                                            FixtureElCalcioStat2023::create([
                                                'fixture_date' => $fechaFormateada,
                                                'fixture_id' => $fixtureData['fixture']['id'],
                                                'fixture_referee' =>  $fixtureData['fixture']['referee'],
                                                'fixture_timestamp' => $fechaHora,
