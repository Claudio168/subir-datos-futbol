...
use App\Models\Colombia\ColombiaApertura2024;
use App\Models\Colombia\ColombiaAperturaStat2024;
use App\Models\Log;
use Illuminate\Support\Facades\Http;
use Carbon\Carbon;
use Twilio\Rest\Client;

class SubirColombiaController extends Controller
{
        ...
        if ($response->ok()) {
            ...
            if (isset($data['response']) && is_array($data['response'])) {
                ...
                foreach ($data['response'] as $fixtureData) {
                    ...
                    if (!ColombiaApertura2024::where('fixture_id', $fixtureData['fixture']['id'])->exists() || !ColombiaAperturaStat2024::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                        ...
                        if (!ColombiaApertura2024::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                            $contadorPartidos++;
                            //se rellena la tabla premierleaguefixture
                            $fixtures = ColombiaApertura2024::create([
                        ...
                        if (!ColombiaAperturaStat2024::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                            ...
                            if ($response2->ok()) {
                                ...
                                if (isset($data2['response']) && is_array($data2['response'])) {
                                    ...
                                    foreach ($data2['response'] as $indice => $fixtureData2) {
                                        if (!ColombiaAperturaStat2024::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                                            $contadorStats++;
                                            ColombiaAperturaStat2024::create([
                                                'fixture_date' => $fechaFormateada,
                                                'fixture_id' => $fixtureData['fixture']['id'],
                                                'fixture_referee' =>  $fixtureData['fixture']['referee'],
                                                'fixture_timestamp' => $fechaHora,
