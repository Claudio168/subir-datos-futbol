...
use App\Models\Alemania\SubirLaBundesligaFixture2023;
use App\Models\Alemania\FixtureLaBundesligaStat2023;
use App\Models\Log;
use Illuminate\Support\Facades\Http;
use Carbon\Carbon;
use Twilio\Rest\Client;

class SubirLaBundesligaFixtureController extends Controller
{
        ...
        if ($response->ok()) {
           ...
           if (isset($data['response']) && is_array($data['response'])) {
            ...
            foreach ($data['response'] as $fixtureData) {
                ...
                if (!SubirLaBundesligaFixture2023::where('fixture_id', $fixtureData['fixture']['id'])->exists() || !FixtureLaBundesligaStat2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                    ...
                    if (!SubirLaBundesligaFixture2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                        $contadorPartidos++;
                        //se rellena la tabla premierleaguefixture
                        $fixtures = SubirLaBundesligaFixture2023::create([
                    ...
                    if (!FixtureLaBundesligaStat2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                        ...
                        if ($response2->ok()) {
                            ...
                            if (isset($data2['response']) && is_array($data2['response'])) {
                                ...
                                foreach ($data2['response'] as $indice => $fixtureData2) {
                                    if (!FixtureLaBundesligaStat2023::where('fixture_id', $fixtureData['fixture']['id'])->exists()) {
                                        $contadorStats++;
                                        FixtureLaBundesligaStat2023::create([
                                            'fixture_date' => $fechaFormateada,
                                            'fixture_id' => $fixtureData['fixture']['id'],
                                            'fixture_referee' =>  $fixtureData['fixture']['referee'],
                                            'fixture_timestamp' => $fechaHora,
